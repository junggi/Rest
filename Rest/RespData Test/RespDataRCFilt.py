...
class RespData(object):
    ...
    def zeroWithFiltFilt(self, data):
        ...
        for i in range(numCols):
            newData[:,i] = data[:,i] - self.lowpass(data[:,i],.2,3)
        return newData
...
##        nearZero = np.std(data) * 0.05
        ...
        for i in range(numCols):
            ...
            decUpperPeaks = self.medfilt(decUpperPeaks, filterWidth)
            decLowerPeaks = self.medfilt(decLowerPeaks, filterWidth)
